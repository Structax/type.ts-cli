...
const generate = async () => {
  ...
  for (const route of routes) {
    const path = route[0]
    const { filePath, method } = route[1] as unknown as { filePath: string; method: string }
    const mod = await import(filePath)
    const api = mod.api
    if (!api || !api.input || !api.handler) continue

    const fnName = toClientFunctionName(method, path)
  ...
  console.log(`✅ Generated client.ts: ${OUT_FILE}`)
}
